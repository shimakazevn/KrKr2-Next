...
namespace TJS // following is in the namespace
{
    ...
    void parser::error(const std::string &msg) {
        ...
        if (ptr) {
            ttstr scriptName = ptr->GetName() ? ptr->GetName() : TJS_W("anonymous");
            int errpos = ptr->GetLexicalAnalyzer() ? ptr->GetLexicalAnalyzer()->GetCurrentPosition() : -1;
            int line = errpos >= 0 ? 1 + ptr->SrcPosToLine(errpos) : -1;
            spdlog::get("tjs2")->critical("Syntax error in script: {} at line {}, pos {}", scriptName.AsNarrowStdString(), line, errpos);
            ttstr wmsg(msg.c_str());
            _yyerror(wmsg.c_str(), ptr, errpos);
        }
    }
    ...
    void tTJSInterCodeContext::Finalize() {
        ...
        if(PropGetter)
            PropGetter->Release(), PropGetter = nullptr;
        if(SuperClassGetter)
            SuperClassGetter->Release(), SuperClassGetter = nullptr;

        if(CodeArea)
            TJS_free(CodeArea), CodeArea = nullptr;
        if(_DataArea) {
...
//---------------------------------------------------------------------------
    ...
    void tTJSInterCodeContext::RegisterFunction() {
        ...
        if(ContextType != ctFunction && // ctExprFunction is not concerned here
           ContextType != ctProperty && ContextType != ctClass) {
            return;
        }
        ...
        switch(node->GetOpcode()) {
            ...
            case parser::token_kind_type::T_SUPER: // 'super'
            {
                ...
                if(Parent && Parent->ContextType == ctProperty) {
                    if((node = Parent->Parent->SuperClassExpr) == nullptr) {
                        _yyerror(TJSCannotGetSuper, Block);
                        return 0;
                    }
                } else {
                    if(!Parent || (node = Parent->SuperClassExpr) == nullptr) {
                        _yyerror(TJSCannotGetSuper, Block);
                        return 0;
                    }
                }
            ...
            case parser::token_kind_type::T_INLINEARRAY: {
                ...
                if(node->GetSize() == 1 && (*(*node)[0])[0] == nullptr) {
                    // the element is empty
                } else {
                    for(tjs_int i = 0; i < nodesize; i++) {
                        _GenNodeCode(frame, (*node)[i], TJS_RT_NEEDED, 0,
                                     tSubParam()); // elements
                    }
                }
...
//---------------------------------------------------------------------------
    ...
    void tTJSInterCodeContext::EnterSwitchCode(tTJSExprNode *node) {
        ...
        if(FrameBase != resaddr) {
            PutCode(VM_CP, NODE_POS);
            PutCode(TJS_TO_VM_REG_ADDR(FrameBase),
                    NODE_POS); // FrameBase points the reference value
            PutCode(TJS_TO_VM_REG_ADDR(resaddr), NODE_POS);
        }
    ...
    void tTJSInterCodeContext::ProcessCaseCode(tTJSExprNode *node) {
        ...
        // process "default :" if node == nullptr.
    ...
    //---------------------------------------------------------------------------
    void tTJSInterCodeContext::DoBreak() {
        // process "break".
    ...
    void tTJSInterCodeContext::EnterTryCode() {
        ...
        PutCode(0, LEX_POS);
        PutCode(TJS_TO_VM_REG_ADDR(FrameBase),
                LEX_POS); // an exception object will be held here
