...
void tTJSNI_BaseWindow::Invalidate() {
    ...
    //	if(LayerManager) LayerManager->SetWindow(nullptr);
...
void tTJSNI_BaseWindow::SetDrawDeviceObject(const tTJSVariant &val) {
    ...
    if(val.Type() == tvtObject) {
        ...
        if(TJS_FAILED(clo.PropGet(0, TJS_W("interface"), nullptr, &iface_v,
                                  nullptr))) {
            TVPConsoleLog(TJS_W("(warn) SetDrawDeviceObject: PropGet(interface) failed, falling back to default"));
            // Don't throw - fall back to no draw device
            return;
        }
        auto *tmpDrawDevice = reinterpret_cast<iTVPDrawDevice *>(
            (tjs_intptr_t)(tjs_int64)iface_v);
        if(!tmpDrawDevice) {
            TVPConsoleLog(TJS_W("(warn) SetDrawDeviceObject: interface pointer is null, falling back to default"));
            // Don't throw - fall back to no draw device
            return;
        }
...
//---------------------------------------------------------------------------
bool tTJSNI_BaseWindow::GetWaitVSync() const { return WaitVSync; }
//---------------------------------------------------------------------------
...
#ifdef USE_OBSOLETE_FUNCTIONS
    TJS_BEGIN_NATIVE_METHOD_DECL(
        /*func. name*/ setLayerPos) // not setLayerPosition
    {
        TJS_GET_NATIVE_INSTANCE(/*var. name*/ _this,
                                /*var. type*/ tTJSNI_Window);
        if(numparams < 2)
