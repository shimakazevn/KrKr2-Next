...
void tTVPFileMedia::NormalizePathName(ttstr &name) {
    // normalize path name
    // Lowercase all characters for case-insensitive path matching.
    // Original code only did this on Windows, but games (e.g. Maitetsu)
    // use case-insensitive paths everywhere. On Android/iOS the filesystem
    // is case-sensitive, so we must normalize to avoid "file not found".
    tjs_char *p = name.Independ();
    while(*p) {
        if(*p >= TJS_W('A') && *p <= TJS_W('Z'))
            *p += TJS_W('a') - TJS_W('A');
...
void tTVPFileMedia::GetLocallyAccessibleName(ttstr &name) {
    ...
    // }
#else // posix
    // NOTE: Removed the Android fast-path that bypassed case-insensitive
    // directory resolution. Games like Maitetsu use case-insensitive paths
    // (e.g. "system/Initialize.tjs" vs "system/initialize.tjs") which fail
    // on Android/iOS's case-sensitive filesystem without this resolution.
    if(!TJS_strncmp(ptr, TJS_W("./"), 2)) {
        ptr += 2; // skip "./"
        newname.Clear();
    }
...
void TVPAutoMountSiblingXP3Archives() {
    ...
    auto getXp3Priority = [](const std::string &name) {
        std::string n = name;
        for(auto &c : n) c = (char)tolower((unsigned char)c);
        if(n.rfind("patch", 0) != 0) {
            // Non-patch archive (data, bgm, voice, video, etc.)
            return std::make_tuple(0, 0, n);
        }
        if(n.rfind("patch_append", 0) == 0) {
            return std::make_tuple(1, 0, n);
        }
        if(n.rfind("patch_data", 0) == 0) {
            return std::make_tuple(2, 0, n);
        }
        // Main patch series: patch.xp3, patch2.xp3, patch3.xp3, patch_vn.xp3
        int num = 1;
        if(n == "patch.xp3") {
            num = 1;
        } else if(n.size() > 9 && n.substr(n.size() - 4) == ".xp3") {
            std::string mid = n.substr(5, n.size() - 9);
            bool isDig = !mid.empty();
            for(char c : mid) if(!isdigit((unsigned char)c)) isDig = false;
            if(isDig) num = std::stoi(mid);
            else num = 999;
        } else {
            num = 999;
        }
        return std::make_tuple(3, num, n);
    };

    std::sort(xp3Names.begin(), xp3Names.end(), [&](const std::string &a, const std::string &b) {
        return getXp3Priority(a) < getXp3Priority(b);
    });
