...
//---------------------------------------------------------------------------
static tjs_uint TVPIndexAutoPathSingle(const ttstr &path) {
    tjs_uint count = 0;
...
void TVPAddAutoPath(const ttstr &name) {
    ...
    if(i == TVPAutoPathList.end()) {
        ...
        if(AutoPathTableInit) {
            TVPAutoPathCache.Clear();
            TVPIndexAutoPathSingle(normalized);
        } else {
...
static tjs_uint TVPRebuildAutoPathTable() {
    ...
    for(it = TVPAutoPathList.begin(); it != TVPAutoPathList.end(); it++) {
        totalcount += TVPIndexAutoPathSingle(*it);
    }
...
ttstr TVPGetPlacedPath(const ttstr &name) {
    ...
    // Check local filesystem first (flat files override archive contents)
    // This allows users to place custom.tjs etc. on disk to override archive versions
    {
        const tjs_char *normCStr = normalized.c_str();
        const tjs_char *delimPos = nullptr;
        for(const tjs_char *p = normCStr; *p; p++) {
            if(*p == TVPArchiveDelimiter) delimPos = p;
        }
        if(delimPos) {
            // Get the archive prefix (e.g. "file://.../data.xp3")
            ttstr archivePrefix(normCStr, (tjs_int)(delimPos - normCStr));
            // Extract the directory containing the archive
            ttstr archiveDir = TVPExtractStoragePath(archivePrefix);
            if(!archiveDir.IsEmpty()) {
                ttstr localPath = archiveDir + storagename;
                if(TVPIsExistentStorageNoSearchNoNormalize(localPath)) {
                    TVPAutoPathCache.Add(name, localPath);
                    return localPath;
                }
            }
        }
