...
// drawdeviceD3D.dll / drawdeviceD3DZ.dll stub
static void TVPRegisterDrawDeviceD3DStub() {
    const tjs_char *stubScript = TJS_W(
        "if (typeof global.D3DLayer == 'undefined') {\n"
        "  class D3DImage {\n"
        "    function D3DImage() {}\n"
        "    function load() {}\n"
        "    function copyFrom() {}\n"
        "    function isEmote() { return false; }\n"
        "  }\n"
        "  class D3DLayer extends Layer {\n"
        "    function D3DLayer(win, par) { super.Layer(win, par); }\n"
        "    var drawPlane = 0;\n"
        "    function calcAffine() {}\n"
        "    function piledCopy() {}\n"
        "    function errorImage() {}\n"
        "  }\n"
        "  D3DLayer.DrawPlaneBoth = 0;\n"
        "  D3DLayer.DrawPlaneFront = 1;\n"
        "  D3DLayer.DrawPlaneBack = 2;\n"
        "  class D3DPicture {\n"
        "    function D3DPicture(layer, img) {}\n"
        "  }\n"
        "  class D3D {\n"
        "    function D3D() {}\n"
        "  }\n"
        "  class DrawDeviceD3D {\n"
        "    function DrawDeviceD3D(w, h) {}\n"
        "  }\n"
        "  global.D3DImage = D3DImage;\n"
        "  global.D3DLayer = D3DLayer;\n"
        "  global.D3DPicture = D3DPicture;\n"
        "  global.D3D = D3D;\n"
        "  global.DrawDeviceD3D = DrawDeviceD3D;\n"
        "}\n"
    );
    TVPExecuteExpression(ttstr(stubScript));
    spdlog::info("Registered D3D / DrawDeviceD3D classes stub");
}

extern void TVPRegisterLZ4FSPlugin();

// windowEx.dll stub — Maitetsu's MainWindow.tjs calls Plugins.link("windowEx.dll")
// for DPI awareness and window management. On mobile these are no-ops.
static void TVPRegisterWindowExStub() {
    spdlog::info("Registered windowEx.dll stub (mobile no-op)");
}
...
void TVPLoadPlugin(const ttstr &name) {
    ...
    if(name == TJS_W("lzfs.dll") || name == TJS_W("lzfs")) {
        TVPRegisterLZ4FSPlugin();
        spdlog::info("Loading Plugin: {} Success (internal)", name.AsStdString());
        return;
    }

    if(name == TJS_W("drawdeviceD3D.dll") || name == TJS_W("drawdeviceD3DZ.dll") ||
       name == TJS_W("drawdeviceD3D") || name == TJS_W("drawdeviceD3DZ")) {
        TVPRegisterDrawDeviceD3DStub();
        spdlog::info("Loading Plugin: {} Success (internal stub)", name.AsStdString());
        return;
    }
    ...
    if(TVPLoadInternalPlugin(pluginName)) {
        ...
        } else if(name == TJS_W("windowEx.dll") || name == TJS_W("windowEx")) {
            TVPRegisterWindowExStub();
        } else if(name == TJS_W("messenger.dll") || name == TJS_W("messenger")) {
            spdlog::info("Loading Plugin: {} skipped (not needed on mobile)", name.AsStdString());
        } else if(name == TJS_W("PackinOne.dll") || name == TJS_W("PackinOne")) {
            spdlog::info("Loading Plugin: {} skipped (not needed on mobile)", name.AsStdString());
        } else if(name == TJS_W("KAGParserEx.dll") || name == TJS_W("KAGParserEx")) {
            spdlog::info("Loading Plugin: {} skipped (not needed on mobile)", name.AsStdString());
        }
    }
}
