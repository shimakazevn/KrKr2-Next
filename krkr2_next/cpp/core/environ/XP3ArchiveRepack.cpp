...
class XP3ArchiveRepackAsyncImpl : ICompressProgressInfo {
public:
...
private:
    ...
    static constexpr int nCodecMethod = 0x30101, // LZMA
        nCodecMethodCopy = 0;
...
static bool CheckIsImage(tTJSBinaryStream *s) {
    ...
    if(!memcmp(header, "\xFF\xD8\xFF", 3)) { // JPEG
        return true;
    }
    if(!memcmp(header, "TLG", 3)) {
        return true;
    }
    if(!memcmp(header, "\x49\x49\xbc\x01", 4)) { // JXR
        return true;
    }
...
XP3ArchiveRepackAsyncImpl::AddTo7zArchive(tTJSBinaryStream *s,
    ...
    if(compressable) {
        ...
        tjs_uint8 header[16];
        s->Read(header, 16);
        if(!memcmp(header, static_cast<const void *>("\x89PNG"), 4)) {
            compressable = false;
            ...
            compressable = false;
        } else if(!memcmp(header, "0&\xB2\x75\x8E\x66\xCF\x11",
                          8)) { // wmv/wma
            compressable = false;
