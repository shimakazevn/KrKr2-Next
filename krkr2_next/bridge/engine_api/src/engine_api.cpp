...
#if defined(__ANDROID__)
    auto android_sink = std::make_shared<spdlog::sinks::android_sink_mt>("krkr2_native");
#endif
...
#if defined(__ANDROID__)
      if (android_sink != nullptr) {
        const auto already_android = std::any_of(
            sinks.begin(), sinks.end(),
            [&](const std::shared_ptr<spdlog::sinks::sink>& sink) {
              return sink.get() == android_sink.get();
            });
        if (!already_android) {
