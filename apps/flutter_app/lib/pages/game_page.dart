...
class _GamePageState extends State<GamePage> with WidgetsBindingObserver {
  ...
  Future<String?> _preflightGamePath(String path) async {
    try {
      ...
      // Accept either startup.tjs in root, data/system/initialize.tjs, or a valid XP3 archive like data.xp3
      ...
      final dataExe = File('$path/data.exe');
      if (!await startup.exists() &&
          !await init.exists() &&
          !await initUpper.exists() &&
          !await dataXp3.exists() &&
          !await dataExe.exists()) {
        return 'Missing startup script in: $path\n'
            '(looked for startup.tjs, data.xp3, and data/system/initialize.tjs)';
      }
    } catch (e) {
