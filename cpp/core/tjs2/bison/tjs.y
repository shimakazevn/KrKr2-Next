...
%parse-param { tTJSScriptBlock *ptr }
%lex-param   { tTJSScriptBlock *ptr }
...
func_def
	: "function" T_SYMBOL					{ ptr->PushContextStack(
	  ...
	  block									{ cc->ExitBlock(); ptr->PopContextStack(); }
;
...
property_handler_setter
	: "setter" "(" T_SYMBOL variable_type ")"	{ ptr->PushContextStack(
												TJS_W("(setter)"),
												ctPropertySetter);
											  cc->EnterBlock();
...
property_handler_getter
	: property_getter_handler_head			{ ptr->PushContextStack(
												TJS_W("(getter)"),
												ctPropertyGetter);
											  cc->EnterBlock(); }
	  block									{ cc->ExitBlock();
...
comma_expr
	: assign_expr								{ $$ = $1; }
	| comma_expr "," assign_expr				{ $$ = cc->MakeNP2(token::T_COMMA, $1, $3); }
;


assign_expr
	: cond_expr									{ $$ = $1; }
	| cond_expr "<->" assign_expr				{ $$ = cc->MakeNP2(token::T_SWAP, $1, $3); }
	| cond_expr "=" assign_expr					{ $$ = cc->MakeNP2(token::T_EQUAL, $1, $3); }
	| cond_expr "&=" assign_expr				{ $$ = cc->MakeNP2(token::T_AMPERSANDEQUAL, $1, $3); }
	| cond_expr "|=" assign_expr				{ $$ = cc->MakeNP2(token::T_VERTLINEEQUAL, $1, $3); }
	| cond_expr "^=" assign_expr				{ $$ = cc->MakeNP2(token::T_CHEVRONEQUAL, $1, $3); }
	| cond_expr "-=" assign_expr				{ $$ = cc->MakeNP2(token::T_MINUSEQUAL, $1, $3); }
	| cond_expr "+=" assign_expr				{ $$ = cc->MakeNP2(token::T_PLUSEQUAL, $1, $3); }
	| cond_expr "%=" assign_expr				{ $$ = cc->MakeNP2(token::T_PERCENTEQUAL, $1, $3); }
	| cond_expr "/=" assign_expr				{ $$ = cc->MakeNP2(token::T_SLASHEQUAL, $1, $3); }
	| cond_expr "\\=" assign_expr				{ $$ = cc->MakeNP2(token::T_BACKSLASHEQUAL, $1, $3); }
	| cond_expr "*=" assign_expr				{ $$ = cc->MakeNP2(token::T_ASTERISKEQUAL, $1, $3); }
	| cond_expr "||=" assign_expr				{ $$ = cc->MakeNP2(token::T_LOGICALOREQUAL, $1, $3); }
	| cond_expr "&&=" assign_expr				{ $$ = cc->MakeNP2(token::T_LOGICALANDEQUAL, $1, $3); }
	| cond_expr ">>=" assign_expr				{ $$ = cc->MakeNP2(token::T_RARITHSHIFTEQUAL, $1, $3); }
	| cond_expr "<<=" assign_expr				{ $$ = cc->MakeNP2(token::T_LARITHSHIFTEQUAL, $1, $3); }
	| cond_expr ">>>=" assign_expr				{ $$ = cc->MakeNP2(token::T_RBITSHIFTEQUAL, $1, $3); }
;
...
logical_or_expr
	: logical_and_expr							{ $$ = $1; }
	| logical_or_expr "||" logical_and_expr		{ $$ = cc->MakeNP2(token::T_LOGICALOR, $1, $3); }
;

logical_and_expr
	: inclusive_or_expr							{ $$ = $1; }
	| logical_and_expr "&&"
	  inclusive_or_expr							{ $$ = cc->MakeNP2(token::T_LOGICALAND, $1, $3); }
;

inclusive_or_expr
	: exclusive_or_expr							{ $$ = $1; }
	| inclusive_or_expr "|" exclusive_or_expr	{ $$ = cc->MakeNP2(token::T_VERTLINE, $1, $3); }
;

exclusive_or_expr
	: and_expr									{ $$ = $1; }
	| exclusive_or_expr "^" and_expr			{ $$ = cc->MakeNP2(token::T_CHEVRON, $1, $3); }
;

and_expr
	: identical_expr							{ $$ = $1; }
	| and_expr "&" identical_expr				{ $$ = cc->MakeNP2(token::T_AMPERSAND, $1, $3); }
;

identical_expr
	: compare_expr								{ $$ = $1; }
	| identical_expr "!=" compare_expr			{ $$ = cc->MakeNP2(token::T_NOTEQUAL, $1, $3); }
	| identical_expr "==" compare_expr			{ $$ = cc->MakeNP2(token::T_EQUALEQUAL, $1, $3); }
	| identical_expr "!==" compare_expr			{ $$ = cc->MakeNP2(token::T_DISCNOTEQUAL, $1, $3); }
	| identical_expr "===" compare_expr			{ $$ = cc->MakeNP2(token::T_DISCEQUAL, $1, $3); }
;

compare_expr
	: shift_expr								{ $$ = $1; }
	| compare_expr "<" shift_expr				{ $$ = cc->MakeNP2(token::T_LT, $1, $3); }
	| compare_expr ">" shift_expr				{ $$ = cc->MakeNP2(token::T_GT, $1, $3); }
	| compare_expr "<=" shift_expr				{ $$ = cc->MakeNP2(token::T_LTOREQUAL, $1, $3); }
	| compare_expr ">=" shift_expr				{ $$ = cc->MakeNP2(token::T_GTOREQUAL, $1, $3); }
;

shift_expr
	: add_sub_expr								{ $$ = $1; }
	| shift_expr ">>" add_sub_expr				{ $$ = cc->MakeNP2(token::T_RARITHSHIFT, $1, $3); }
	| shift_expr "<<" add_sub_expr				{ $$ = cc->MakeNP2(token::T_LARITHSHIFT, $1, $3); }
	| shift_expr ">>>" add_sub_expr				{ $$ = cc->MakeNP2(token::T_RBITSHIFT, $1, $3); }
;
...
mul_div_expr
	: unary_expr								{ $$ = $1; }
	| mul_div_expr "%" unary_expr				{ $$ = cc->MakeNP2(token::T_PERCENT, $1, $3); }
	| mul_div_expr "/" unary_expr				{ $$ = cc->MakeNP2(token::T_SLASH, $1, $3); }
	| mul_div_expr "\\" unary_expr				{ $$ = cc->MakeNP2(token::T_BACKSLASH, $1, $3); }
	| mul_div_expr_and_asterisk unary_expr		{ $$ = cc->MakeNP2(token::T_ASTERISK, $1, $2); }
;
...
unary_expr
	...
	| "#" unary_expr							{ $$ = cc->MakeNP1(token::T_SHARP, $2); }
	| "$" unary_expr							{ $$ = cc->MakeNP1(token::T_DOLLAR, $2); }
	...
	| incontextof_expr "instanceof" unary_expr	{ $$ = cc->MakeNP2(token::T_INSTANCEOF, $1, $3); }
	| incontextof_expr "in" unary_expr			{ $$ = cc->MakeNP2(token::T_IN, $1, $3); }
	| "(" "int" ")" unary_expr					{ $$ = cc->MakeNP1(token::T_INT, $4); }
	| "int" unary_expr							{ $$ = cc->MakeNP1(token::T_INT, $2); }
	| "(" "real" ")" unary_expr					{ $$ = cc->MakeNP1(token::T_REAL, $4); }
	| "real" unary_expr							{ $$ = cc->MakeNP1(token::T_REAL, $2); }
	| "(" "string" ")" unary_expr				{ $$ = cc->MakeNP1(token::T_STRING, $4); }
	| "string" unary_expr						{ $$ = cc->MakeNP1(token::T_STRING, $2); }
;

incontextof_expr
	: priority_expr								{ $$ = $1; }
	| priority_expr "incontextof"
		incontextof_expr						{ $$ = cc->MakeNP2(token::T_INCONTEXTOF, $1, $3); }
;

priority_expr
	: factor_expr								{ $$ = $1; }
	| "(" expr ")"								{ $$ = $2; }
	| priority_expr "[" expr "]"				{ $$ = cc->MakeNP2(token::T_LBRACKET, $1, $3); }
	| func_call_expr							{ $$ = $1; }
	...
	| "."										{ lx->SetNextIsBareWord(); }
	  T_SYMBOL								    { tTJSExprNode * node = cc->MakeNP0(token::T_CONSTVAL);
												  ...
												  $$ = cc->MakeNP1(token::T_WITHDOT, node); }
;
...
factor_expr
	...
	| "void"									{ $$ = cc->MakeNP0(token::T_VOID); }
	| inline_array								{ $$ = $1; }
	| inline_dic								{ $$ = $1; }
	| const_inline_array						{ $$ = $1; }
	| const_inline_dic							{ $$ = $1; }
	| "/="										{ lx->SetStartOfRegExp(); }
	  T_REGEXP									{ $$ = cc->MakeNP0(token::T_REGEXP);
...
array_elm
	: /* empty */						{ $$ = cc->MakeNP1(token::T_ARRAYARG, NULL); }
...
dic_elm
	: expr_no_comma "," expr_no_comma	{ $$ = cc->MakeNP2(token::T_DICELM, $1, $3); }
